...
# extracting the moves of each player and total time of each player
...
chess_games_data['Moves'] = pgn_column.apply(extract_move)
...
transformed_train$white_result <- factor(transformed_train$white_result, levels = c("loss", "win"))
y_numeric <- as.numeric(transformed_train$white_result) - 1

# Explainer
explainer_ANN <- explain(model = ANN_final, 
                         data = transformed_train[, -which(names(transformed_train) == "white_result")],
                         y = y_numeric,
                         label = "ANN Model")

# Compute permutation-based feature importance for the ANN model
fi_ANN <- model_parts(explainer = explainer_ANN, 
                      type = "raw",
                      loss_function = loss_accuracy)

# Plot the feature importance
plot(fi_ANN, max_vars = 20)
